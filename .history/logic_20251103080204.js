...
async function saveSelection(deptCode, qId, value) {
  const key = getStorageKeyFor(deptCode);
  const saved = JSON.parse(localStorage.getItem(key) || "{}");
  if (value) saved[qId] = Number(value);
  else delete saved[qId];

  localStorage.setItem(key, JSON.stringify(saved)); // optional local backup

  // NEW: Save to server
  const payload = {
    username: sessionStorage.getItem("dmi_username"),
    dept: deptCode,
    questionId: qId,
    value: value,
