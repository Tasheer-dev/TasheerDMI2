...
function initAssessmentPage() {
  ...
  script.onload = () => {
    ...
    document.getElementById("pageTitle").textContent = `Tasheer DMI – Assessment Report for ${displayName}`;
...
function getNextLevelRecommendation(q, selectedValue) {
  ...
  // Remove numbering from the recommendation text
  return nextChoice ? nextChoice.text.replace(/^\d+\.\s*/, "") : "N/A";
}
...
function finalizeAssessment() {
  ...
  // Set dynamic report header
  const displayName = sessionStorage.getItem("dmi_displayName");
  document.getElementById("reportHeaderTitle").textContent = `DMI Final Assessment Report for ${displayName}`;
  ...
  tbody.innerHTML = deptData.questions.map(q => {
    ...
    const label = choice ? choice.text.replace(/^\d+\.\s*/, "") : "-"; // Remove numbering from selected answer
    const nextRec = getNextLevelRecommendation(q, val);
    const color = getColorByScore(val * 20); // Scale 1-5 to 20-100 for color function

    // Find the maturity band name for the current score
    const maturityBand = window.COMMON_MATURITY_BANDS.find(band => val >= band.range[0] && val <= band.range[1]);
    const levelName = maturityBand ? maturityBand.name.split(":")[1].trim() : "-";

    return `<tr>
      <td>${q.title}</td>
      <td>${label}</td>
      <td style="text-align: center; background-color: ${color.replace(/,1\)/, ', 0.2)')};">${levelName}</td>
      <td>${nextRec}</td>
      <td></td> <!-- Findings column - to be filled by user in the comments box -->
    </tr>`;
  }).join("");

  // Add a final row for the general comments/findings
  tbody.innerHTML += `<tr>
    <td colspan="4" style="font-weight: bold; text-align: right;">General Findings / Comments:</td>
    <td>${savedComments}</td>
  </tr>`;

  // This section is now unused due to table update, but kept for safety
