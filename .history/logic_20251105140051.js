...
/* Add this new helper function somewhere in logic.js (e.g., near getNextLevelRecommendation) */
function getLevelInWords(score) {
    switch (String(score)) {
...
function generateDetailedReport(deptCode, deptData, totalScore, maxScore) {
  const saved = loadSavedScores(deptCode);
  
  // 1. Update Dynamic PDF Title
  document.title = `Tasheer DMI – Assessment Report for ${deptData.title}`;

  // Update report headings
  document.getElementById("reportTitle").textContent = `Assessment Report for ${deptData.title}`;
  document.getElementById("reportHeader").textContent = deptData.title;

  // Update total score section
  document.getElementById("overallScore").textContent = totalScore;
  document.getElementById("maxScore").textContent = maxScore;
  document.getElementById("finalScorePercentage").textContent = ((totalScore / maxScore) * 100).toFixed(1) + "%";

  // 2. Add Department Name after "Overall Digital Maturity Score"
  document.getElementById("overallScoreHeading").innerHTML = `Overall Digital Maturity Score <span class="dept-name-tag">(${deptData.title})</span>`;

  // 3. Remove the sentence "Based on an overall score of 41 out of 70 possible points."
  // This sentence is typically generated here. Assuming the element ID is 'overallScoreText'
  // If the sentence is in your HTML, you can delete it there. If it's dynamic, remove the logic.
  // For safety, let's assume the element has a placeholder:
  const overallScoreTextElement = document.getElementById("overallScoreText");
  if (overallScoreTextElement) {
    overallScoreTextElement.style.display = 'none'; // or overallScoreTextElement.remove();
  }
  
  // =========================================================================================
  // 4. Update the Details Table Columns
  // =========================================================================================
  const tbody = document.getElementById("detailedScoreBody");

  // Get comments to include in the Findings column
  const commentsKey = getStorageKeyFor(deptCode) + "_comments";
  ...
  tbody.innerHTML = deptData.questions.map(q => {
    ...
    // 5. Recommendation (remove numbering)
    const recommendationText = label.replace(/^\d+\.\s*/, ''); // Remove "1. ", "2. ", etc.

    // Get Findings/Comments for this question
    const findings = savedComments[q.id] || "";
    
    // 6. Level (in words)
    const levelInWords = getLevelInWords(val);
  ...
  // Update table header in the HTML report page to match:
  // <th>Question</th><th>Answer</th><th>Level (in words)</th><th>Recommendation</th><th>Findings</th>
  
  // Note: The structure of the rec.innerHTML block below (Final Recommendations) is redundant 
  // with the new 'Recommendation' column in the detailed table and could be removed entirely
  // from the report HTML if you only want the detailed table.
  const rec = document.getElementById("finalRecommendations");
  rec.innerHTML = "<ul>" + deptData.questions.map(q => {
    const val = saved[q.id];
    const choice = q.choices.find(c => String(c.value) === String(val));
    const recommendationText = choice ? choice.text.replace(/^\d+\.\s*/, '') : "-";
    return `<li>${recommendationText}</li>`;
  }).join("") + "</ul>";
