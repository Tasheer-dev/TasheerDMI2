...
// Normalize quiz metadata to avoid stale maxScore/totalQuestions
if (!Array.isArray(deptData.questions)) deptData.questions = [];
const realTotalQs = deptData.questions.length;
const realMax = realTotalQs * 5;
if (deptData.totalQuestions !== realTotalQs) deptData.totalQuestions = realTotalQs;
if (deptData.maxScore !== realMax) deptData.maxScore = realMax;
...
score cluculator 
 */
function calculatePercentAndTotal(savedMap, deptData) {
  // Sum only IDs that actually exist in this quiz
  const validIds = new Set(deptData.questions.map(q => q.id));
  let totalPoints = 0;
  deptData.questions.forEach(q => { totalPoints += Number(savedMap[q.id] || 0); });

  const max = deptData.questions.length * 5;
  const percent = max ? Math.round((totalPoints / max) * 100) : 0;
  return { totalPoints, percent, max };
}
...
function updateLiveScore(deptCode, deptData) {
  ...
  // Count answered from valid questions only
  const validIds = new Set(deptData.questions.map(q => q.id));
  let answered = 0;
  deptData.questions.forEach(q => { if (saved[q.id]) answered++; });

  const { percent } = calculatePercentAndTotal(saved, deptData);

  document.getElementById("liveScorePercentage").textContent = percent + "%";
  document.getElementById("liveScoreStatus").textContent =
    `${answered} / ${deptData.totalQuestions} completed`;
...
function finalizeAssessment() {
  ...
  // Percent using the same function as Live
  const { totalPoints, percent, max } = calculatePercentAndTotal(saved, deptData);
  ...
  document.getElementById("maxScoreText").textContent = max;

  // Area bars + details (unchanged)
  ...
  tbody.innerHTML = deptData.questions.map(q => {
    ...
    const color = getColorByScore(val * 20);
    return `<tr>
      <td>${q.id}</td>
