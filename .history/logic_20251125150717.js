...
function computeDynamicAreaScores(deptData, savedMap) {
  ...
  // ---------------------------
  const sectionScores = deptData.sections.map(sec => {
    let total = 0;
    let max = sec.questions.length * 5;
    ...
    return {
      area: sec.name,          // Use section title as bar label
      percent: percent
    };
  });

  // ---------------------------
  // 2. AREA-BASED SCORING (optional)
  // ---------------------------
  const areaDict = {};
  const areaOrder = [];
  ...
  deptData.questions.forEach(q => {
    ...
    if (!areaDict[group]) {
      areaDict[group] = { total: 0, max: 0 };
      areaOrder.push(group);
    }

    areaDict[group].total += Number(savedMap[q.id] || 0);
    areaDict[group].max += 5;
  });

  const areaScores = areaOrder.map(g => {
    const a = areaDict[g];
    return {
      area: g,
      percent: Math.round((a.total / a.max) * 100)
    };
  });
  ...
  // ---------------------------
  return {
    sections: sectionScores,   // Used for chart
    areas: areaScores,         // Optional
    overall: overall
  };
}
...
const labels = summary.sections.map(s => s.area).concat(["Overall Score"]);
const data = summary.sections.map(s => s.percent).concat([summary.overall]);
