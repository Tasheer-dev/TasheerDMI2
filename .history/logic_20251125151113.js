...
let finalDoughnutInstance = null;

function renderFinalDoughnut(percent) {
    const ctx = document.getElementById("finalDoughnutChart");

    if (!ctx) return;

    // Destroy old chart if exists
    if (finalDoughnutInstance) finalDoughnutInstance.destroy();

    const completed = percent;
    const remaining = 100 - percent;

    finalDoughnutInstance = new Chart(ctx, {
        type: "doughnut",
        data: {
            labels: ["Completed", "Remaining"],
            datasets: [{
    ...
    // Update numeric percentage in the middle
    const label = document.getElementById("finalDoughnutText");
