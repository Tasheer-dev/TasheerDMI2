...
function getEvidenceStorageKey() {
  if (!CURRENT_DEPT_CODE) return null;
  return `tasheer_dmi_${CURRENT_DEPT_CODE}_evidence`;
}

// Called when user types / changes notes
function handleEvidenceChange(e) {
  const ta = e.target;
  const qId = ta.dataset.questionId;
  const key = getEvidenceStorageKey();
  if (!key || !qId) return;

  let evidenceMap = {};
  try {
    evidenceMap = JSON.parse(localStorage.getItem(key)) || {};
  } catch (err) {
    console.warn("Failed to parse evidence map from localStorage", err);
  }

  evidenceMap[qId] = ta.value || "";
  localStorage.setItem(key, JSON.stringify(evidenceMap));
}

// Called after building form to restore saved notes
function loadEvidenceForForm() {
  const key = getEvidenceStorageKey();
  if (!key) return;
  ...
  inputs.forEach(input => {
    ...
    if (qId && evidenceMap[qId] !== undefined) {
      input.value = evidenceMap[qId];
    }
  });
}
...
// -----------------------------------------------------------------------------
// Global default maturity bands (used for Company Overall and Divisions)
// If a specific dept quiz defines its own `maturityBands`, that will override
// these. Otherwise these common bands are used.
// -----------------------------------------------------------------------------
window.COMMON_MATURITY_BANDS = window.COMMON_MATURITY_BANDS || [
  {
    name: "0–25% (Ad-hoc)",
    range: [0, 25],
    description: "Reactive, largely manual operations; limited documentation and visibility."
  },
  {
    name: "26–50% (Developing)",
    range: [26, 50],
    description: "Foundational digitization; some standard processes; uneven adoption and automation."
  },
  {
    name: "51–75% (Defined/Managed)",
    range: [51, 75],
    description: "Standardized and documented processes; regular reporting; growing automation."
  },
  {
    name: "76–90% (Measured/Advanced)",
    range: [76, 90],
    description: "Integrated automation; dashboards; proactive, data-driven management."
  },
  {
    name: "91–100% (Optimized)",
    range: [91, 100],
    description: "Predictive, AI-enabled operations; continuous improvement embedded."
  }
];
